...
main :: IO ()
main = do
    contents <- readFile "/home/miney/code/haskell/adventofcode/2022/12.txt"
    let m = enumerate $ lines contents
    let [s,e] = findEnds m
    let as = findAs m
    let g = getGraph $ clean m
    -- part 1
    print $ fromJust $ shortestDistance g s e
    -- part 2
    print $ minimum $ mapMaybe (\start -> shortestDistance g start e) as
